 ...
 * buffering all <code>ITask</code> generated by packets received from the server.
 * <p>
 ...
 * <p>
 * When a new <code>RealTimeTask</code> is added, <code>TaskManager</code>
 * automatically discards the older version of the task that is considered 'equal'
 * to the newly added one. This guarantees that there is only the newest version
 * of the same <code>RealTimeTask</code> in the task queue.
 * 
...
public class TaskManager extends Manager {
	...
	/**
	 * Add the given task to the <code>TaskManager</code> for later execution. If there is an
	 * earlier <code>RealTimeTask</code> that is considered 'equal' as the newly given one,
	 * the older version is automatically removed before the new one is added. If the given
	 * task is earlier than the 'equal' one in the queue, the given task is discarded.
	 * @see <code>RealTimeTask</code> for 'equal' determination details.
	 * @param task The <code>ITask</code> to be added.
	 * @return True if the task is successfully added. False if the given task is discarded.
	 */
